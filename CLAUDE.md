...
> **고객 = 수입 신선식품 바이어(MD) "Ian"** — 발주 결정권을 직접 쥔 사람.

- **누구**: 대형마트·식품 이커머스(쿠팡·마켓컬리·이마트 등)의 수입 신선식품 구매 담당. "무엇을, 얼마나, 언제 발주할지"를 본인이 결정·실행.
- **KPI**: 매출·마진·**결품률·폐기율**. 결품도 폐기도 "바이어 발주 실수"로 귀결 → 양쪽 다 본인 책임.
- **고통**: 통관 일정이 깜깜이라 **감(感)으로 발주 도박**. 늦을까 봐 과발주 → 한꺼번에 들어와 폐기 / 적게 발주 → 결품.
- **설계 원칙**: 모든 기능은 "Ian이 보고 → 바로 발주/이관을 누를 수 있는가"로 판단한다. 관제용 정보가 아니라 **발주 의사결정 도구**여야 한다. 예: 조기경보는 "위험"에서 끝나지 않고 "그래서 언제·얼마 발주하라"까지 줘야 한다(주문 시점 제안).
- **왜 기존 솔루션이 이 고객을 안 풀었나**: Windward·Portcast의 고객은 화주·포워더라 관심이 ETA까지. "FC 입고 후 바이어가 팔 수 있는 시점"은 그들 관심 밖. → FreshFlow는 **바이어 관점**으로 한국 공공데이터를 발주 결정에 연결한다.
...
2. **[권장] 화면 문구를 바이어 관점으로** — 현재 화면은 관제 톤. "Ian 바이어가 발주를 누르는" 시점으로 카피 조정(섹션 0.5). 기능 로직은 그대로, 라벨·안내문만.
...
### Phase 2-F: 시나리오 자동화 (미구현 — Phase 3급)
> 외부 평가 조언(3번 후반). 기상청 태풍예보·항만 파업 뉴스·UNI-PASS 마비 징후를 크롤링/NLP → 슬라이더 자동 위기화 + 예상지연 선행 리포팅. **주의**: 출처·신뢰도 투명 공개 필수(정직성).
